...
시뮬 세션 상태와 API 컨트랙트의 무결성을 지킨다 — DB 직쓰기·DT 직접 import·schema 임의 변경이 가장 큰 함정이다.

<!--
== Tradeoff (카파시 원칙 #17) ==
세션 상태를 in-memory에 두면 DB 영속성을 포기하는 대신 일관성·테스트 격리를 얻는다.
DT 호출을 어댑터로 강제하면 직통 호출 1줄 편의를 포기하는 대신 모킹 가능성과 책임 경계를 얻는다.
-->

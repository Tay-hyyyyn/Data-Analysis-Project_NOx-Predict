...
환경 분리(dev/prod/data)와 secret 격리를 지킨다. 평문 secret·`latest` 태그·환경 혼용이 가장 큰 함정.

<!--
== Tradeoff (카파시 원칙 #17) ==
compose 파일을 환경별로 분리하면 단일 파일의 단순함을 포기하는 대신 dev에서 prod DB 접근 같은 환경 혼용 사고를 차단한다.
이미지 태그를 명시 버전으로 고정하면 `latest`의 자동 갱신 편의를 포기하는 대신 재기동 시점의 의도치 않은 버전 풀을 막는다.
-->

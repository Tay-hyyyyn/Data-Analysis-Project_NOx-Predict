...
τ/임계치/dt SoT가 `simulation/config.py`에 모이도록 유지한다. 도메인 객체와 임계 정의가 분산되면 시뮬과 백엔드 임계가 어긋난다.

<!--
== Tradeoff (카파시 원칙 #17) ==
임계/τ를 단일 `config.py`로 모으면 영역별 미세 튜닝의 자유를 포기하는 대신 cross-영역 일관성을 얻는다.
Ridge·LGB 앙상블을 유지하면 단일 모델의 단순함을 포기하는 대신 OOD 외삽 능력 + 비선형 미세조정을 둘 다 얻는다.
-->
